...
class AutonomousSteeringAgent(SteeringAgent):
    def calculate_driving_decision(self,xte, road_angle, distance_around_track):
        ...
        heading_error = self.agent_orientation - road_angle
        heading_error = (heading_error + math.pi) % (2 * math.pi) - math.pi
        K_heading = 1.9
        K_xte = 0.0025
        steering_wheel_position = -K_heading * heading_error - K_xte * xte
        steering_wheel_position = max(-1, min(1, steering_wheel_position))
        curve_severity = abs(heading_error) + abs(xte) / 450
        target_speed = 1.0 - 0.6 * min(1, curve_severity) 
...
if __name__=="__main__":
    ...
    if agent==Agents.NEURAL:
        ...
        # You might need more trials than 200, and you might need several random restarts (just re-run the program yourself to do this).
            # Step 2: Evaluate its initial fitness
    ...
    print("Initial fitness:", round(best_fitness, 2))
    for generation in range(500):
        mutation = np.random.normal(loc=0, scale=0.05, size=(chromosome_length))
        chromosome_test = chromosome + mutation
        driving_agent = NeuralSteeringAgent(
            track_layout.preferred_start_point[0],
            track_layout.preferred_start_point[1],
            None,
        ...
        if fitness > best_fitness:
            ...
            print("Generation", generation, "New best fitness:", round(best_fitness, 2))
        else:
            print("Generation", generation, "Fitness", round(fitness, 2), "Best", round(best_fitness, 2))
