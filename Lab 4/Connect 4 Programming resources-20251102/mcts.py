...
def expand_mcts_tree_once(mcts_node):
    ...
    while mcts_node.children is not None:
        ...
        if terminal_state_children!=[]:
            ...
            if None in ucts:
                unvisited_children=[mcts_node.children[i] for i, x in enumerate(ucts) if x == None]
    ...
    if not mcts_node.is_terminal_node():
        moves = mcts_node.board.valid_moves()
        assert len(moves) > 0
        assert mcts_node.children == None
    ...
    while mcts_node is not None:
        ...
        if victorious_player != 0 and mcts_node.board.get_player_who_just_moved() == victorious_player:
            # The AND in the line above ensures that, if this was a win for player 1, as we back propagate up the tree, only alternating layers receive win+=1.
            # If it was player 2 who won then the alternating would happen too, but be the other way around (e.g. hit the odd layers instead of the even ones, or vice-versa).
            # So this means the wins at each layer of the tree refer to the opposite player winning compared to the layer before.
            # This means that in the "selection" phase above, when it chooses the best uct score at each layer, it is choosing the best move for the
            # player whose turn it is; rather like minimax.  Clever stuff!
            # Also note we match mcts_node.board.get_player_who_just_moved() == victorious_player, because that player is the one who's just won
            # or lost; not the person whose turn it is to move next!
