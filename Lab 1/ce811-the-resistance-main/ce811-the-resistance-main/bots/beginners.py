...
import random
class Paranoid(Bot):
    """An AI bot that tends to vote everything down unless specific conditions apply."""
    ...
    def vote(self, team):
        if self in team and not self.spy:
            return True
        if self.game.tries == 5 and not self.spy:
            return True
...
#         return True 
        ...
        # def sabotage(self):
        # if self.game.turn==1:
        #     self.say("not sabotaging - it's the first mission.")
        #     return False
        # if len(self.game.team)==2:
        #     self.say("not sabotaging - mission has only 2 players")
        #     return False
        # self.say("sabotaging - Normal Behaviour.")
        # return True 
